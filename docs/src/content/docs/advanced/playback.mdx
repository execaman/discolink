...
Every playback method exists on both `Player` (takes an additional `guildId`) and `Queue` (operates on itself). They're identical in behaviour — use whichever is more convenient.
...
**`stop()` vs `destroy()`** — `stop()` retains the current track of queue but removes it from Lavalink.

**`resume()` behaviour** — if `queue.stopped` is `true`, `resume()` calls `jump(0)` to restart the current track. Otherwise the default is to unpause.
