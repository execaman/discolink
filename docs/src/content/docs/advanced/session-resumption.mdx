...
<Aside type="caution">
  `autoSync` and `relocateQueues` are part of `player.options`, if you've set them to `true` at construction, set them
  to `false` before implementing your own behavior for them.
</Aside>
