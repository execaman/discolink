...
```
←  previousTracks[-2]  ·  previousTracks[-1]  |  (current) track[0]  ·  tracks[1]  →
```
...
queue.stopped; // true when queue has a current track but Lavalink doesn't (resume() will start it)
...
queue.destroyed; // true when this instance is no longer in QueueManager
...
<Aside type="note">
  The default `fetchRelatedTracks` returns `[]`. Autoplay does nothing until you implement it. This function should
  absolutely not throw an error and only communicate its result by returning an empty or populated array of tracks.
</Aside>
...
await queue.addRelated(specificTrack); // use a specific track as reference
```
...
await player.queues.destroy(guildId, "No human listeners"); // via manager
```
...
Full API: [Queue](/discolink/api/classes/Queue.Queue.html)
