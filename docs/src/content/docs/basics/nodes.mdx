...
Nodes are passed in options to the `Player` constructor. `clientId` is injected automatically from `player.init()` — you don't pass it yourself.
...
node.connected; // boolean (socket open, not signaled ready by Lavalink yet)
...
player.nodes.supports("plugin", "lavalyrics-plugin", "us-east"); // boolean
```
...
Full option types: [NodeOptions](/discolink/api/interfaces/Typings.NodeOptions.html)
