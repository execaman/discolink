...
| `voiceTimeout`       | `10_000`     | Milliseconds to wait for Discord voice gateway events before timing out                |
| `voiceReconnect`     | `true`       | Auto-reconnect on recoverable voice close codes (4004, 4006, 4011)                     |
...
player.on("voiceDisconnect", (voice, details) => {
  // Fires when auto-reconnect fails after a voice close.
  // details: { code, reason, byRemote, error? }
});
```
...
player.on("queueFinish", (queue, reason, error) => {
  // Queue is empty and autoplay returned nothing.
  // reason: 'finished' | 'noRelated' | 'requestFailed'
  // error: present when reason is 'requestFailed'
